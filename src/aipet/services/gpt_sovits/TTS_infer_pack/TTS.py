...
class TTS:
    ...
    def _get_ref_spec(self, ref_audio_path):
        # 兼容性修复：避免在 Windows 上因缺失 torchcodec / ffmpeg 解码器而导致 torchaudio.load 报错
        # 优先使用 soundfile 读取音频并转换为 PyTorch 张量
        try:
            import soundfile as sf
            raw_audio_np, raw_sr = sf.read(ref_audio_path)
            # 如果是单声道，sf.read 返回一维数组，转换为 (1, num_samples)
            if len(raw_audio_np.shape) == 1:
                raw_audio_np = np.expand_dims(raw_audio_np, axis=0)
            else:
                # 转置为 (num_channels, num_samples) 以与 torchaudio.load 的格式保持一致
                raw_audio_np = raw_audio_np.T
            raw_audio = torch.from_numpy(raw_audio_np)
        except Exception as e:
            # 若 soundfile 读取失败，则回退到原始的 torchaudio.load 方式
